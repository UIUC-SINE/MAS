...
\eqref{eq:fast_cost} for a particular configuration $\bm d$: The multiplication
...
With that, the complexity of overall SBS algorithm is dominated by the inversion
of $\bm \Psi \triangleq \left( \widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm d} +
\lambda \widetilde{\bm \Sigma}_x^{-1} \right) \in \mathbb C ^{SN^2 \times SN^2}$
that is performed in each iteration. Complexity of a standard inversion
...
of complexity $O_i = O(S^3N^2)$ \cite{kamaci2017}, providing a speed-up of $N^4$, which is huge for large image sizes. The total complexity of the SBS algorithm can
be written as:
$$
...
focal plane reconstructions. We first provide an intuition for why SBS chooses
an alternative configuration that improves on the focal plane configuration, and
then test the algorithm under a variety of scenarios including varying SNRs,
number of spectral components, and spectral component separation in wavelength.
We use photon sieve as the diffractive element in our simulations, which offers
improved PSFs than Fresnel zone plates \cite{kipp2001sharper}.

For the first experiment, we simulate a scenario with two spectral components
that are close to each other in wavelenght, shown as separate colors in Figure
\ref{fig:results}(a). We first run SBS to find a measurement configuration, and
then compare its image reconstruction performance with focal plane
configuration. We run SBS on a set of uniformly spaced candidate plane
locations that are centered around the focal planes. We set the prior covariance
matrix $\bm \Sigma_x$ such that it promotes global smoothness in each image, and
use noise level of \SI{15}{\decibel} SNR for each measurement. The final
measurement configuration selected by SBS is given in Figure \ref{fig:results}(d),
where the two focal planes are marked with red and green bars.

We use the MAP estimation framework given in Section 2 as the image
reconstruction algorithm for both the focal plane and SBS configurations for
consistency, where we take $\bm x_0=0$. We search for the best $\lambda$ for
the focal plane reconstructions, and used the same $\lambda$ for both running
SBS, and reconstructing the components with SBS and focal plane configurations.
Figures \ref{fig:results}(b) and \ref{fig:results}(c) show the spectral
component reconstructions for focal plane configuration, and Figures
\ref{fig:results}(e) and \ref{fig:results}(f) does the same for SBS
configuration. Table \ref{tab:ssims} provides the structural similary (SSIM) and
peak signal-to-noise ratio (PSNR) values of the reconstructions.


% \begin{itemize}
% \item Wavelengths: $\lambda_1 = $ \SI{33.400}{\centi\meter}, $\lambda_2 = $ \SI{33.402}{\centi\meter}
% \item Candidate plane locations - $\bm d$: Equally spaced 30 locations plus
% focal planes, 10 copy of each
% \item Final number of planes - $M$: 12
% \item Sieve parameters: Diameter: \SI{10}{\centi\meter}, number of zones: 3570
% \item Measurement noise: \SI{15}{\decibel} SNR
% \end{itemize}

% In this experiment, we initialized SBS with 12 repeated
% measurements at 32 candidate planes for a total of $C = 384$ candidate
% measurements. We allowed SBS to run until only $M = 12$ measurements remain and
% show the final measurement configuration selected in Figure
% \ref{fig:results}(d). This plot shows that SBS has selected a set of
...
Our intuition on why SBS chooses out of focus planes over the focal planes is
...
\begin{figure}[htb]
  ...
  \end{minipage}
  \caption{Reconstruction SSIMs for various settings. SBS reconstruction SSIM and
  focal plane reconstruction SSIM are shown in orange and blue.}
  \label{fig:ratio}
\end{figure}

For the second experiment, we repeat what we did for $S = 2, 3, 4$ uniformly
spaced spectral components under a variety of noise levels and spectral
component separations.  In Figure \ref{fig:ratio}, we plot the mean SSIM of the
reconstructions obtained from measurements at focal planes (blue) and
measurements at planes selected by SBS (orange).  The SBS reconstructions
generally outperform the focal plane reconstructions except when the spectral
component separation is large enough (around XXX nm).  With sufficient
separation, we have found that SBS converges to a final measurement
configuration near the focal planes.
