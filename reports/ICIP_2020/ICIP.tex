...
$\widetilde{\bm A}_{\bm d}$ and $\widetilde{\bm \Sigma}_x^{-1}$. Since $\widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm d}$ does not have to be
recomputed at every SBS iteration, but rather can be s


% Now, we derive the computational complexity of the fast SBS implementation and
% compare it with the general implementation. The computational complexity of
% evaluating the cost at \eqref{eq:fast_cost} for a particular measurement
% configuration $\bm{d}$ comes from two places: The multiplication $\widetilde{\bm
% A}_{\bm d}^H \widetilde{\bm A}_{\bm d}$, and the inversion $\left(
% \widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm d} + \lambda \widetilde{\bm
% \Sigma}_x^{-1} \right)^{-1}$, whose complexities we denote as $O_m$ and $O_i$,
% respectively.
%
% The product $\widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm d}$ can be
% rewritten as sum of products $\widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm
% d} = \sum_{d \in \bm{d}} \widetilde{\bm A}_{d}^H \widetilde{\bm A}_{d}$ where
% $\widetilde{\bm A}_{d} \in \mathbb R^{N^2 \times SN^2}$ is the measurement matrix
% with diagonalized blocks at the distance $d$ such that $\widetilde{\bm A}_{\bm d} = \begin{bmatrix}
% \widetilde{\bm A}_{d_1}^H & \dots & \widetilde{\bm A}_{d_C}^H
% \end{bmatrix}^H$.
% With this property, we can simply precompute all $\widetilde{\bm A}_{d}^H
% \widetilde{\bm A}_{d}$ during algorithm initialization and use the results to
% form $\widetilde{\bm A}_{\bm d}^H \widetilde{\bm A}_{\bm d}$ as needed.
%
% Since each $\widetilde{\bm A}_{d}$ has shape $(1 \times S)$ with diagonal blocks
% of size $N^2$, the overall complexity for precomputing $\widetilde{\bm A}_{d}^H \widetilde{\bm A}_{d}$ for all $C$ candidate locations is
%
% $$
