...
% of complexity $O_i = O(S^3N^2)$ \cite{kamaci2017}, providing a speed-up of $N^4$, which is significant for large image sizes. The total complexity of the SBS algorithm can
% be written as:
% $$
...
reconstructions obtained from measurements taken at focal planes. We first
provide an intuition for why SBS chooses an alternative configuration that
improves on the focal plane configuration, and then test the algorithm under a
variety of scenarios including varying SNRs, number of spectral components, and
spectral component separation in wavelength. We use a photon sieve as the
...
that are close to each other in wavelength, shown as separate colors in Figure
\ref{fig:results}(a). We first run SBS to find a measurement configuration, and
then compare the quality of its reconstruction with the focal plane
configuration reconstruction. We run SBS on a set of uniformly spaced candidate plane
locations centered around the focal planes and choose a prior covariance
matrix $\bm \Sigma_x$ that promotes global smoothness in each image. We use the
MAP estimation framework given in Section 2 as the image reconstruction
algorithm for both the focal plane and SBS configurations for consistency, where
we take $\bm x_0=0$. We search for the best $\lambda$ for the focal plane
reconstructions, and used the same $\lambda$ for both running SBS, and
reconstructing the components with SBS and focal plane configurations.

The final measurement configuration selected by SBS is given in Figure
...
expense of a less sharp in focus PSF.
...
For the second experiment, we repeat the first experiment for $S = 2, 3, 4$
uniformly spaced spectral components under a variety of noise levels and
spectral component separations.  In Figure \ref{fig:ratio}, we plot the mean
...
component separation is large enough (around 10 depth-of-focus).  With
sufficient separation, we have found that SBS converges to a final measurement
