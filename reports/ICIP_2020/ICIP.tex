...
distances from the lens \cite{attwood2017x}, as shown in Figure
\ref{fig:diff_lens}(a).
...
that is performed in each iteration. The complexity of a standard inversion
...
that as the source wavelengths get very close to each other, each focal plane
get higher contamination from the other source; as a result, it gets harder for
the deconvolution algorithm to disentangle the sources. Therefore, the algorithm
prefers out of focus planes where relative contamination from off focus sources
are smaller, and it is easier to disentangle the sources.
...
We repeat this experiment for $S = 2, 3, 4$ uniformly spaced spectral components
under a variety of noise levels and spectral component separations.  In Figure
\ref{fig:ratio}, we plot the mean SSIM of the reconstructions obtained from
measurements at focal planes (blue) and measurements at planes selected by SBS
(orange).  The SBS reconstructions generally outperform the focal plane
reconstructions except when the spectral component separation is large enough
(around XXX nm).  With sufficient separation, we have found that SBS converges
to a final measurement configuration near the focal planes.
