...
reconstruction qualit by making measurements away from these focal positions.
Additionally, we propose a greedy backward selection algorithm for automatically
discovering such a measurement configuration.
...
system.  Consider a polychromatic source that has $S$ two dimensional spectral components
$\bm{x}_1, \dots, \bm{x}_S$.  Using a moving detector, we make $M$ two
dimensional measurements $\bm{y}_1, \dots, \bm{y}_M$ at different distances from
the lens. Due to linearity, each measurement is a superposition of blurred
