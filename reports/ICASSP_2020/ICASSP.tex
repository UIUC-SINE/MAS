...
quality, so we would like to find the best configuration of measurement locations that
...
However, it is possible to improve on the reconstruction quality in some
situations by choosing a measurement configuration which includes location away from these focal positions, as we show
later in the paper. We propose a greedy backward selection algorithm for
