...
geographic surveying, astronomy, and solar physics [REFS].
A common method to perform spectral imaging is to use a set of optical filters
to selectively pass light of specific wavelengths of interest.
...
to show the measurement selection algorithm. Later, we present a reformulation
of the algoritm for fast implementation. Then, we analyse the complexity of this
and compare with the naive implementation. In the numerical results, we show
that the proposed algorithm leads better reconstructions than a naive
configuration.
...
\begin{figure}[htb]
  \begin{minipage}[b]{1\linewidth}
    \centering
    \centerline{\includegraphics[width=9.5cm]{drawing}}
  \end{minipage}
...
A similar property and be exploited for accelerated inversion of BCCB blocks. \cite{kamaci2017}
...
speedup of $N^4$, which is can be significant for large images.
...
\section{Conclusion}
We have presented a method for determining the optimal measurement configuration
in multispectral imaging with a diffractive lens. We developed an efficient
implementation that achieves significant speed up. We demonstrated the potential
improvement achieved by the method over a straightforward selection of
