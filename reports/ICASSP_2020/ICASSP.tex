...
is fixed and does not vary with SBS iteration.
...
% \bm{A}^H_{\bm{d}} \bm{A}_{\bm{d}} =
% \left.
%   \vphantom{\begin{bmatrix}x \\ x\end{bmatrix}}
%   \smash{
%     \underbrace{
%       \begin{bmatrix}
%         \bm{A}_{d_1,1} & \bm{A}_{d_1,S}\\
%         \bm{A}_{d_{|\bm{d}|},1} & \bm{A}_{d_{|\bm{d}|},S}\\
%       \end{bmatrix}
%     }_{S}
%   }
% \right\} |\bm{d}|
...
where we use the fact that $\sum_{|\bm{d}| =M}^C |\bm{d}|^2 = O(C^3)$ and $\sum_{|\bm{d}| = M}^C |\bm{d}|= O(C^2)$.
% $$
% \begin{aligned}
%   & \sum_{|\bm{d}| = M}^C |\bm{d}|^2 \\
%   &= \frac{1}{6}(C - M + 1)(2C^2 + 2CM + C + M(2M - 1)) \\
%   &= O(C^3)
% \end{aligned}
% $$

% $$
% \sum_{|\bm{d}| = M}^C |\bm{d}|= \frac{1}{2}(C - M + 1)(C + M) = O(C^2)
% $$
