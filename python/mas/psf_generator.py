...
def get_aperture_func(sieve):
    """
    Function that takes the sieve structure and outputs a vectorized function
    which outputs the binary sieve mask a(x,y) given the coordinate matrices x and y

    Args:
        sieve (PhotonSieve): a PhotonSieve instance holding sieve parameters

    Returns:
    a vectorized function which outputs the binary sieve mask a(x,y) given the
    coordinate matrices x and y

    """
    inner = sieve.structure[0]['inner_radius']
    outer = sieve.structure[-1]['outer_radius']
    outer_radii = np.zeros(len(sieve.structure))
    for i, zone in enumerate(sieve.structure):
        outer_radii[i] = zone['outer_radius']

    def a_func(x, y):
        radius = np.sqrt(x**2 + y**2)
        # return true if point falls inside hole
        if radius >= inner and radius < outer:
            white_zone = sieve.structure[np.searchsorted(outer_radii,radius)]
...
class PhotonSieve():
    ...
    Attributes:
        ...
        a (def): vectorized function that outputs the binary sieve mask a(x,y)
        mask (ndarray): sampled sieve aperture
"""
