function [X, error, iter, flag] = cgconvtikPS_p3d(X0,max_it,tol,nu,b,Code_mtx,gamma_,gamma_G)
...
Tmp1 = nu * Tmp1;
for i=1:s
    X03d(:,:,i) = indexer(X0,i,1,L1);
end
Gx_ = fftshift(ifftn(ifftshift(gamma_G.*fftshift(fftn(ifftshift(X03d))))));
for i=1:s
    Gx(1+(i-1)*L1:i*L1,:) = Gx_(:,:,i);
end
Tmp1 = Tmp1 + Gx;
...
%     iter
    ...
    Tmp1 = nu * Tmp1;
    for i=1:s
        p3d(:,:,i) = indexer(p,i,1,L1);
    end
    Gp_ = fftshift(ifftn(ifftshift(gamma_G.*fftshift(fftn(ifftshift(p3d))))));
    for i=1:s
        Gp(1+(i-1)*L1:i*L1,:) = Gp_(:,:,i);
    end
    Tmp1 = Tmp1 + Gp;
