function [IOut,paramsout]= tldecon_p3d_cg(I,noisy_Im,psfs,paramsin,I_diff,Code_mtx)

% 3D Sparsifying Transform regularized PSSI deconvolution with 3D Patches
% Measurements are obtained using a random mask
...
for kp=1:num
    ...
    if(kp==1)
        ...
        Lb3=fftn(IMout2Z); gamma_G=real(Lb3);
        gamma_G = fftshift(gamma_G);
    ...
    if(kp==1)
        ...
        ChHhy = La2*Code_mtx .* block_ifft2(my_mul(my_herm(gamma_H, aa), f_Ip, aa),aa);
    end
    
    b = ChHhy + PhWhB;
    tol = 1e-7;  max_it = 200;
    
    [Im, error, iterCG, flagCG] = cgconvtikPS_p3d(Im,max_it,tol,La2,b,Code_mtx,gamma_,gamma_G);
