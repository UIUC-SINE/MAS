function [IOut,paramsout]= tldecon_f3d(I,noisy_Im,psfs,paramsin,I_diff)
...
elseif(strcmp(init,'meas'))
    for i=1:p
        ...
        sc = (max(max(abs(y))));
        sc = 1; % TODO: why there is such a line?
        y = y/sc;
...
elseif(strcmp(init,'mean'))
    ...
    for i= 2:p-1
        if f(i) < max(d) && f(i) > min(d)
            ...
            Im(1+(i-1)*aa:i*aa, 1:aa) = ((d(j1)-f(i))*indexer(noisy_Im,j2,1,aa) + (f(i)-d(j2))*indexer(noisy_Im,j1,1,aa))/(d(j1)-d(j2));
        elseif f(i) > max(d)
...
for kp = 1:num
    ...
    for i=1:p
        X(1+(i-1)*aa:i*aa, 1:aa) = reshape(XH(i,:),aa,aa);
    end
    ...
    for i=1:p
        c(1+(i-1)*aa:i*aa, 1:aa) = reshape(XH(i,:),aa,aa);
    end
        
    %/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/%/
    Fc = block_fft2(c,aa,aa);
%     
    if kp == 1
        gamma_H = block_fft2(psfs, psfsize, aa);
        gamma_H = real(gamma_H);
        ...
        FHhy = La2*my_mul(my_herm(gamma_H, aa), f_noisy_Im, aa);
        
        gamma = La2*gamma_ + kron(eye(p),ones(aa));
        gammainv = my_inv(gamma,p);
    end
    
    
    Im = block_ifft2(my_mul(gammainv,FHhy+Fc,aa),aa);
